...
test('create instance with rules where selector is a global class', function () {
    var sheet = jss.createStyleSheet({a: {float: 'left'}}, {named: false})
    ok(sheet.rules.a)
...
test('accessible rule via selector even if named: true', function () {
    var sheet = jss.createStyleSheet({bla: {float: 'left'}}, {named: true})
    ok(sheet.rules.bla)
...
test('attach/detach', function () {
    var sheet = jss.createStyleSheet({abc: {float: 'left'}}).attach()
...
test('addRule/getRule on attached sheet', function () {
    var sheet = jss.createStyleSheet(null, {named: false}).attach()
    var rule = sheet.addRule('aa', {float: 'left'})
...
test('toString unnamed', function () {
    var sheet = jss.createStyleSheet({a: {float: 'left', width: '1px'}}, {named: false})
    sheet.attach()
...
test('toString named', function () {
    jss.uid.reset()
    var sheet = jss.createStyleSheet({a: {float: 'left', width: '1px'}})
    sheet.attach()
